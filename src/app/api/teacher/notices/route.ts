...
const createSchema = z.object(announcementFields);
const updateSchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("update"), noticeId: z.string().min(1).max(200), ...announcementFields }),
...
export async function POST(request: Request) {
  ...
  try {
    const id = await createAnnouncement(user, parsed.data);
...
export async function PATCH(request: Request) {
  ...
  try {
    if (parsed.data.action === "update") {
      const { noticeId, action: _action, ...input } = parsed.data;
