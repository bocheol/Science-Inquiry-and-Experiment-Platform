...
const schema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("archive"), teamId: z.string(), confirmation: z.string().max(100) }),
  z.object({ action: z.literal("restore"), teamId: z.string() }),
]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (input.action === "archive") await archiveTeam(user.id, input.teamId, input.confirmation);
