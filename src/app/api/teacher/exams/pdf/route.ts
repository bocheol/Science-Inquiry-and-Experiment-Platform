...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await getExamSetForPdf(examSetId, studentId, user.id);
    const pdf = await buildExamPdf(data, answers);
    const suffix = answers ? "answer-key" : "papers";
    const filename = `science-exam-${data.clubId ? "club" : "class"}-${data.clubId ?? data.classNumber}-${suffix}.pdf`;
    return new Response(new Uint8Array(pdf), {
      headers: {
        "Cache-Control": "no-store",
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${filename}"; filename*=UTF-8''${encodeURIComponent(`${data.activityLabel}-${data.title}-${answers ? "교사용답안" : "시험지"}.pdf`)}`,
        "X-Content-Type-Options": "nosniff",
      },
    });
  } catch (error) {
    const status = error instanceof ExamServiceError ? error.status : 400;
    return Response.json({ message: error instanceof ExamServiceError ? error.message : "PDF를 만들지 못했습니다. 잠시 후 다시 시도해 주세요." }, { status });
  }
}
