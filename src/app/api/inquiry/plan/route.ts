...
const patchSchema = z.object({ cycleId: z.string().trim().min(1).max(200), planId: z.string(), fieldKey: z.string(), value: z.unknown(), expectedValue: z.unknown().optional() });
const submitSchema = z.discriminatedUnion("action", [
  z.object({ cycleId: z.string().trim().min(1).max(200), planId: z.string().min(1).max(200), action: z.literal("submit") }),
  z.object({ cycleId: z.string().trim().min(1).max(200), planId: z.string().min(1).max(200), action: z.literal("restore"), revisionId: z.string().min(1).max(200) }),
]);
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    await savePlanField(parsed.data.planId, parsed.data.fieldKey, parsed.data.value, user.id, parsed.data.expectedValue, parsed.data.cycleId);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json({ message: userFacingMessage(error, "저장하지 못했습니다. 잠시 후 다시 시도해 주세요.") }, { status: 409 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    await authorizePlan(user.id, parsed.data.planId);
    if (parsed.data.action === "submit") await submitPlan(parsed.data.planId, user.id, parsed.data.cycleId);
    else await restorePlanRevision(parsed.data.planId, parsed.data.revisionId, user.id, parsed.data.cycleId);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json({ message: userFacingMessage(error, "제출하지 못했습니다. 잠시 후 다시 시도해 주세요.") }, { status: 400 });
  }
}
