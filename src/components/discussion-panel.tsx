...
export function DiscussionPanel({ sessionId, cycleId, currentUserId, members, readOnly: readOnlyProp = false, canSummarize = false, active = true }: { sessionId: string; cycleId?: string; currentUserId: string; members: Array<{ id: string; name: string }>; readOnly?: boolean; canSummarize?: boolean; active?: boolean }) {
  ...
  return <section className="discussion-panel stack" aria-label="팀 대화와 활동 기록">
    ...
    {tab === "meeting" ? <>
      ...
      {!readOnly?<form className="stack discussion-note" onSubmit={e=>{e.preventDefault();void save(parentId?'supplement':'meeting');}}><h3>{parentId?'기존 기록에 보완 남기기':'대면 기록 남기기'}</h3><label className="label">실제 활동 날짜<input type="date" className="input" max={today()} value={noteDate} disabled={Boolean(parentId)} onChange={e=>setNoteDate(e.target.value)} required/></label>{!parentId?<fieldset disabled={busy}><legend>함께 참여한 팀원</legend><div className="toolbar-group">{members.map(m=><label key={m.id}><input type="checkbox" checked={participants.includes(m.id)} onChange={e=>setParticipants(p=>e.target.checked?[...p,m.id]:p.filter(id=>id!==m.id))}/>{m.name}</label>)}</div></fieldset>:null}<label className="label" htmlFor={`note-${sessionId}`}>나눈 이야기와 결정한 내용</label>{!parentId?<MeetingAudioInput sessionId={sessionId} cycleId={viewCycleId} value={note} onChange={setNote} disabled={busy}/>:null}<textarea id={`note-${sessionId}`} className="textarea" rows={8} maxLength={16000} disabled={busy} value={note} onChange={e=>setNote(e.target.value)} placeholder="순서나 문장 형식에 얽매이지 않고 적어 주세요. 누가 제안했는지, 무엇을 결정했고 무엇이 남았는지 함께 적으면 좋습니다."/><p className="section-subtitle">원문을 먼저 저장하고 AI가 활동 날짜에 맞춰 정리합니다. 보완은 원문을 덮어쓰지 않고 별도로 남습니다.</p><div className="toolbar-group"><button className="button" disabled={busy||!note.trim()}>{busy?'원문 저장·AI 정리 중…':'저장하고 AI 정리'}</button>{parentId?<button type="button" className="button secondary" onClick={()=>setParentId(null)}>새 기록으로 전환</button>:null}</div></form>:null}
