  ...
  return (
    <section className="card card-body cycle-analysis-panel">
      ...
      {data.session.cycles.map((cycle) => <details key={cycle.id} open={cycle.status === "active"} className="cycle-history-item">
        ...
        <div className="cycle-history-content">{cycle.analysis ? <><div className="cycle-documents"><SnapshotDocument title="고정된 계획서" formData={cycle.analysis.documents.plan.formData} definition={cycle.analysis.documents.plan.configDefinition} /><SnapshotDocument title="고정된 보고서" formData={cycle.analysis.documents.report.formData} definition={cycle.analysis.documents.report.configDefinition} memberRoles={cycle.analysis.documents.report.memberRoles} /><SnapshotMaterials requests={cycle.analysis.documents.materialRequests} /></div><AnalysisBody key={cycle.analysis.id} analysis={cycle.analysis} draftOwnerId={currentUserId} editable={audience === "student" && cycle.status === "active" && cycle.analysis.analysisType === "intermediate" && cycle.analysis.isCurrent} onSaved={() => router.refresh()} /></> : <div className="empty-state">아직 이 회차의 AI 분석이 없습니다.</div>}</div>
