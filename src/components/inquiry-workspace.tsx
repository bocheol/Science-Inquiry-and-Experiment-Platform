...
type Tab = "chat" | "records" | "plan" | "materials" | "journal" | "report" | "exam" | "evaluation";
  ...
  return (
    <div className="stack">
      <section className="team-banner">
        <div><h1>{data.team.activityName ?? `${data.team.classNumber}반`} · {data.team.name}</h1><p>{data.session.selectedTopic || "AI와 탐구 주제를 찾고 있어요"}</p></div>
      ...
      <nav className="tabs" aria-label="탐구 메뉴">
        ...
        {!data.team.clubId ? <button className={`tab ${tab === "materials" ? "active" : ""}`} onClick={() => setTab("materials")}>🧪 준비물 신청</button> : null}
        ...
        {!data.team.clubId ? <><button className={`tab ${tab === "exam" ? "active" : ""}`} onClick={() => setTab("exam")}>🎤 시험 결과</button>
        <button className={`tab ${tab === "evaluation" ? "active" : ""}`} onClick={() => setTab("evaluation")}>⭐ 자기·동료평가</button></> : null}
      </nav>
      <div className="card workspace-panel">
        <div hidden={tab !== "records"}><DiscussionPanel sessionId={data.session.id} currentUserId={currentUserId} members={data.members} active={tab === "records"} /></div>
